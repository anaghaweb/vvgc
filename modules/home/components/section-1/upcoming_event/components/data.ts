export const upcoming_events = [  
  
  // ANANDA DANA

   {
    heading: "Ananda Dana",
    subtitle: "A fundraiser for Anandam, India",
    date: ["11th April 2026"],
    time: "3:30PM - 6:00PM",
    imageArray:[
     {
        url:"https://res.cloudinary.com/dixkqgqsi/image/upload/v1773751033/vvgc/images/2026/april/ananda_dana_0411.jpg",
        alt:"ananda danam image",
        id:1
    },
    
  ],
    description: [`All Are Welcome To Participate With Family Friends And a musical evening `,
    ],
    videoUrl: "",
    details:[`1:00 PM : Sri Venkateswara Abhisheka/Sri Nava Graha Shanthi Homam and Sani Graha Shanthi Homam Nava Graha Abhisheka Sani Graha Abhisheka, Sani Dev Thel /Thaila Abhisheka Sani chalisa Aarati Sri Vishnu Sahasra Nama Chanting Aarati and Mantra Pushpa`,
      `3:30PM - 6:00 PM : Smt Gayathri Venkataraghavan Music Performance`,
      `5:00 PM : Sri Gomatha Pooja Aarati`
    ],
    sponsorLinks: [
        {
        url: `https://scan.page/7b00ZP`,
        name: "To attend please register"
      },
      // { url: "https://bit.ly/vvgc-maharudram", name: "Register Here" },
      // { url: "https://paybee.io/@vvgc@73", name: "Make a donation" },
    ],
  },
  // MAHA RUDRA YAGNAM
  {
    heading: "Maha Rudra Yagnam 2026",
    subtitle: "At this event 121 Ritwiks reciting the Sri Rudra 11 times for a total of 1331 times along with Rudra Abhishekam with 11 kinds of Dravyams. This will be followed by Rudra Homam",
    date: ["18th April 2026"],
    time: "From Morning 7:00AM - 1:30PM",
    imageArray:[
       {
        url:"https://res.cloudinary.com/dixkqgqsi/image/upload/v1775300124/vvgc/images/2026/april/kanchivasa_0418.jpg",
        alt:"kanchivasa image",
        id:1
    },
     {
        url:"https://res.cloudinary.com/dixkqgqsi/image/upload/v1770527473/vvgc/images/2026/april/maha_rudra_yagnam.jpg",
        alt:"maharudra yagnam image",
        id:2
    },
    
  ],
    description: [`All Are Welcome To Participate With Family Friends And Get Blessings Followed By Maha Prasadam `,
    ],
    videoUrl: "",
    details:[],
    sponsorLinks: [
        {
        url: `https://vvgc.org/events/event_661`,
        name: "Click here for complete details"
      },
      { url: "https://bit.ly/vvgc-maharudram", name: "Register Here" },
      { url: "https://paybee.io/@vvgc@73", name: "Make a donation" },
    ],
  },  
  //VVGC DEVELOPMENT FUNDRAISER
   {
    heading: "Chitra Pournami Pooja",
    subtitle: "Temple Development Fundraiser Event",
    date: ["1st & 2nd May"],
    time: "1st May - 7:00PM, 2nd May - 11:00AM",
    imageArray:[
     {
        url:"https://res.cloudinary.com/dixkqgqsi/image/upload/v1775543914/vvgc/images/2026/may/fundraiser_0501_02.jpg",
        alt:"fundraiser image",
        id:1
    },    
  ],
    description: [`All Are Welcome To Participate With Family and Friends `,
    ],
    videoUrl: "",
    details:[],
    sponsorLinks: [
        {
        url: `https://paybee.io/quickpay.html?handle=vvgc&ppid=9`,
        name: "click here to donate"
      },
      // { url: "https://bit.ly/vvgc-maharudram", name: "Register Here" },
      // { url: "https://paybee.io/@vvgc@73", name: "Make a donation" },
    ],
  },

  // Sri Meenakshi Sundareswarar Kalyanam Utsavam
  {
    heading: "Sri Meenakshi Sundareswarar Kalyanam Utsavam",
    subtitle: "",
    date: ["2nd May"],
    time: "12:30PM - 2:00PM",
    imageArray:[
     {
        url:"https://res.cloudinary.com/dixkqgqsi/image/upload/v1775543920/vvgc/images/2026/may/meenakshi_sundareshwar_thirukalyana_utsavam_0502.jpg",
        alt:"meenakshi sundareshwar utsavam",
        id:1
    },    
  ],
    description: [`All Are Welcome To Participate With Family and Friends`,
    ],
    videoUrl: "",
    details:[],
    sponsorLinks: [
        {
        url: `https://paybee.io/quickpay.html?handle=vvgc&ppid=52#campaignList`,
        name: "Sponsor this event"
      },
      // { url: "https://bit.ly/vvgc-maharudram", name: "Register Here" },
      // { url: "https://paybee.io/@vvgc@73", name: "Make a donation" },
    ],
  },
  //HANUMAN JAYANTHI
  {
    heading: "Chandramana Sri Hanuman Jayanthi Celebrations",
    subtitle: "",
    date: ["12th May"],
    time: "7:00PM",
    imageArray:[
     {
        url:"https://res.cloudinary.com/dixkqgqsi/image/upload/v1775543913/vvgc/images/2026/may/chandramana_hanuman_jayanthi_0512.jpg",
        alt:"hanuman jayanthi",
        id:1
    },    
  ],
    description: [`All Are Welcome To Participate With Family and Friends`,
    ],
    videoUrl: "",
    details:[],
    sponsorLinks: [
        {
        url: `https://paybee.io/quickpay.html?handle=vvgc&ppid=52#optionList`,
        name: "Click here to donate"
      },
      // { url: "https://bit.ly/vvgc-maharudram", name: "Register Here" },
      // { url: "https://paybee.io/@vvgc@73", name: "Make a donation" },
    ],
  },

  //23RD YEAR ANNIVERSARY
  {
    heading: "VVGC 23rd Year Anniversary Celebrations",
    subtitle: "",
    date: ["6th June"],
    time: "Starts at 9:00AM",
    imageArray:[
     {
        url:"https://res.cloudinary.com/dixkqgqsi/image/upload/v1775543977/vvgc/images/2026/june/vvgc_23rd_anniversary.jpg",
        alt:"VVGC 23rd Year Anniversary Celebrations",
        id:1
    },    
  ],
    description: [`All Are Welcome To Participate With Family and Friends`,
    ],
    videoUrl: "",
    details:[],
    sponsorLinks: [
        {
        url: `https://paybee.io/@vvgc@64`,
        name: "Click here to donate"
      },
      // { url: "https://bit.ly/vvgc-maharudram", name: "Register Here" },
      // { url: "https://paybee.io/@vvgc@73", name: "Make a donation" },
    ],
  },
];

