export const upcoming_events = [  
  //SHIVARATRI
  {
    heading: "Maha Shivaratri Mahotsavam",
    subtitle: "Special Event Shiva Parvati (Girija) Kalyanam",
    date: ["15th Febraury"],
    time: "From Morning 10:00AM",
    imageArray:[
     {
        url:"https://res.cloudinary.com/dixkqgqsi/image/upload/v1770109476/vvgc/images/2026/feb/shivaratri_2026_withqrcode.jpg",
        alt:"shivratriimage1",
        id:1
    },
     
     {
        url:" https://res.cloudinary.com/dixkqgqsi/image/upload/v1770526999/vvgc/images/2026/feb/shivaratri_1.jpg",
        alt:"shivaratriimage3",
        id:3
    },
    {
        url:"https://res.cloudinary.com/dixkqgqsi/image/upload/v1770013412/vvgc/images/2026/feb/mahashivaratri_weekend_event.jpg",
        alt:"shivaratriimage2",
        id:2
    },
  ],
    description: [`All Are Welcome To Participate With Family Friends And Get Blessings Followed By Maha Prasadam `,
    ],
    videoUrl: "",
    details:[],
    sponsorLinks: [
      //   {
      //   url: `https://vvgc.org/events/event_661`,
      //   name: "Click here for complete details"
      // },
      // { url: "https://bit.ly/vvgc-maharudram", name: "Register Here" },
      // { url: "https://paybee.io/@vvgc@73", name: "Make a donation" },
    ],
  },

  //BHAGWATI JAGRAN
  {
    heading: "Bhagwati Jagran",
    subtitle: "Priti Bhoj & Mata ki Chowki",
    date: ["Fri 20th Feb 2026"],
    time: "Evening 7:00 PM",
    imageArray:[
     {
        url:"https://res.cloudinary.com/dixkqgqsi/image/upload/v1770963675/vvgc/images/2026/feb/bhagwati_jagran_20260220.jpg",
        alt:"bhagwati_jagran_image",
        id:1
    },
    
  ],
    description: [`All Are Welcome To Participate With Family Friends And Get Blessings Followed By Maha Prasadam `,
    ],
    videoUrl: "",
    details:[],
    sponsorLinks: [
      //   {
      //   url: `https://vvgc.org/events/event_661`,
      //   name: "Click here for complete details"
      // },
      // { url: "https://bit.ly/vvgc-maharudram", name: "Register Here" },
      // { url: "https://paybee.io/@vvgc@73", name: "Make a donation" },
    ],
  },
  
  {
    heading: "Maha Rudra Yagnam 2026",
    subtitle: "At this event 121 Ritwiks reciting the Sri Rudra 11 times for a total of 1331 times along with Rudra Abhishekam with 11 kinds of Dravyams. This will be followed by Rudra Homam",
    date: ["18th April 2026"],
    time: "From Morning 7:00AM - 1:30PM",
    imageArray:[
     {
        url:"https://res.cloudinary.com/dixkqgqsi/image/upload/v1770527473/vvgc/images/2026/april/maha_rudra_yagnam.jpg",
        alt:"shivratriimage1",
        id:1
    },
    
  ],
    description: [`All Are Welcome To Participate With Family Friends And Get Blessings Followed By Maha Prasadam `,
    ],
    videoUrl: "",
    details:[],
    sponsorLinks: [
        {
        url: `https://vvgc.org/events/event_661`,
        name: "Click here for complete details"
      },
      { url: "https://bit.ly/vvgc-maharudram", name: "Register Here" },
      { url: "https://paybee.io/@vvgc@73", name: "Make a donation" },
    ],
  },
  
];

