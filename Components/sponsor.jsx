...
export default function SponsorEvent({ data, index }) {
    ...
    return (
        ...
        <Card key={index} className={` flex flex-col w-full sm:w-full m-auto h-auto my-2`}>
        ...
        <CardContent key={index} className="grid grid-cols-10 gap-x-1 gap-y-3 my-5">
          {data.startTime && data.endTime && data.description ?
            ...
            : data.startTime && !data.endTime ?
              ...
              : data.startTime && data.endTime && !data.description ? ""
                : <div className="col-span-10 text-[13px] md:text-base">{data.description}<br/></div>
          }
        ...
        {/* sponsor event Link */}
        <Card className="flex flex-col p-2 rounded w-full md:max-w-fit border-slate-400 border-solid shadow-inner bg-gray-100 md:bg-transparent " >
        {data.sponsorLink &&
          ...
          <Link className="flex sm:max-w-fit justify-start items-center " href={data.sponsorLink} target="_blank">
            ...
            <FaExternalLinkAlt style={{ color: 'indigo' }} /></Link>}
